...
class FL_SysInfo_Reporter {
    ...
    protected function get_info() {
        ...
        return array(
            ...
            'file_logger_info' => array(
                'title' => '파일 로거 정보',
                'data' => array(
                    '로그 디렉토리'        => FL::get_log_dir(),
                    '<span title="로그 디렉토리에 쓰기 가능 여부">로그 디렉토리 쓰기 가능</span>' => is_writable( FL::get_log_dir() ) ? '예' : '아니오',
                    '<span title="파일 시스템에서의 접근 권한">로그 파일 권한</span>' => $this->get_log_permissions(),
                    '디버그 모드'           => FL::is_debug_mode() ? '활성화' : '비활성화',
                    '<span title="보관할 최대 로그 파일 개수">최대 로그 파일</span>'        => get_option( 'fl_max_log_files', 30 ),
                    '<span title="로그 파일 교체 주기">로그 로테이션</span>'         => get_option( 'fl_log_rotation', 'daily' ) === 'daily' ? '매일' : get_option( 'fl_log_rotation', 'daily' ),
                ),
            ),
        );
    }
    ...
    public static function display() {
        $reporter = new self();
        ?>
        <div class="fl-sysinfo-container">
            <p class="fl-sysinfo-actions">
                <button type="button" class="button button-primary" id="fl-copy-sysinfo">
                    <span title="시스템 정보를 컴퓨터의 임시 저장소에 복사">클립보드에 복사</span>
                </button>
                <span class="fl-copy-message" style="display:none;color:green;margin-left:10px;">
                    복사됨!
                </span>
            </p>
            
            <div id="fl-sysinfo-content" class="fl-sysinfo-grid">
                <?php
            ...
            <div style="display:none;">
                <textarea id="fl-sysinfo-text" readonly><?php echo esc_textarea( $reporter->get_plain_text_report() ); ?></textarea>
            </div>
        </div>
        
        <style>
        .fl-sysinfo-container {
            margin-top: 20px;
        }
        .fl-sysinfo-grid {
            display: grid;
        ...
        .fl-sysinfo-grid .postbox .inside {
            padding: 0;
            margin: 0;
        }
        .fl-sysinfo-grid table.widefat {
            border: 0;
            margin: 0;
            border-radius: 0;
            box-shadow: none;
        }
        .fl-sysinfo-grid table.widefat thead,
        .fl-sysinfo-grid table.widefat tfoot {
            display: none;
        }
        .fl-sysinfo-grid table.widefat tr:first-child td,
        .fl-sysinfo-grid table.widefat tr:first-child th {
            border-top: 0;
        }
        .fl-sysinfo-grid table.widefat th {
            font-weight: 400;
            color: #666;
            padding: 10px 12px;
            width: 40%;
        ...
        .fl-sysinfo-grid table.widefat td {
            padding: 10px 12px;
            color: #2c3338;
            word-break: break-word;
        }
        .fl-sysinfo-grid table.striped > tbody > tr:nth-child(odd) {
            background-color: #f6f7f7;
        }
        .fl-sysinfo-grid table.striped > tbody > tr:nth-child(even) {
            background-color: #fff;
        }
        .fl-sysinfo-actions {
            margin-bottom: 15px;
        }
        .fl-sysinfo-actions .button {
            height: 32px;
            line-height: 30px;
            padding: 0 12px;
        }
        /* Hover effects */
        .fl-sysinfo-grid .postbox:hover {
        ...
        jQuery(document).ready(function($) {
            $('#fl-copy-sysinfo').on('click', function() {
                var button = $(this);
                var text = $('#fl-sysinfo-text').val();
                var message = $('.fl-copy-message');
